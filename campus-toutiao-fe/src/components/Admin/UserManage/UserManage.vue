        ...
        methods: {
            ...
            batchDel() {
                this.$confirm('此操作将会删除多个用户, 是否继续?', '提示', {
                    ...
                    for (let i = 0; i < this.multipleSelection.length; i++) {
                        delUserIdList.push(this.multipleSelection[i].id);
                    }
                    // console.log(delNewsIdList);
                    this.$http({
                        method: 'post',
                        url: '/api/admin/batchDelUser',
                        data: {
                            delUserIdListStr: JSON.stringify(delUserIdList)
                        }
                    }).then((response) => {
            ...
            getData(currentPage, pageSize) {
                this.$http({
                    ...
                    if (response.data.toutiaoCode === 0) {
                        this.tableData = response.data.toutiaoData.userList;
                        for (let i = 0; i < this.tableData.length; i++) {
                            this.tableData[i].gender = this.tableData[i].gender === 0 ? '男' : '女';
                        }
                    }
                }).catch((error) => {
                    console.log(error);
            ...
            getUserCount() {
                this.$http({
                    method: 'get',
                    url: '/api/admin/getAllUserCount'
                }).then((response) => {
                    if (response.data.toutiaoCode === 0) {
                        this.userTotal = response.data.toutiaoData.userTotal;
                    }
                }).catch((error) => {
                    console.log(error);
                    this.$Message.error({
